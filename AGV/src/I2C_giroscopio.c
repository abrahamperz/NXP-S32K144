/*
 * IIC por hardware, utilizando el módulo LPI2C0
 * las posiciones se quedan guardas en un apuntador *datos_I2C[3]={&I2C1,&I2C2,&I2C3}
 * posteriormente se envia por I2C
 *
 */
...
unsigned char i_i2c=0;

unsigned int* I2C1=0;
unsigned int* I2C2=0;
unsigned int* I2C3=0;

unsigned int* datos_I2C[3]={&I2C1,&I2C2,&I2C3};
...
void LPIT0_Ch2_IRQHandler(void)
{
  ...
  bandera++;
             ...
             *I2C1=roll_x=0.98*(roll_x+(float)((gyro_x)/131)*dt)+0.02*(roll_x_acc); //250/32768=1/131


             *I2C2= pitch_y=0.98*(pitch_y+(float)((gyro_y)/131)*dt)+0.02*(pitch_y_acc);

             //Integracion respecto del tiempo para calcular el YAW

             *I2C3=yaw_z=yaw_z+(float)((gyro_z)/131)*dt;
...
void LPUART2_RxTx_IRQHandler (void)
{
                ...
                if ((LPUART2->STAT & (1<<23))==(1<<23))   //Tx empty flag
                {
